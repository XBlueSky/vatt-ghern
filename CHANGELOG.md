...
- LICENSE (MIT), CHANGELOG, CONTRIBUTING, SECURITY, PR template, issue
  templates, Dependabot config, CODEOWNERS, `.editorconfig` (PR #8).
- Repo social preview image (1280×640) generated by
  `scripts/build-social.mjs`; `npm run social` rebuilds it. Upload via
  GitHub Settings → Social preview.
- `main` branch protection: PRs required, `quality` CI must pass,
  force-push and branch deletion disabled, stale reviews auto-dismissed,
